...
public class JandexCollectorTestCase {
    ...
    public void testNoUsage() throws Exception {
        ...
        scanClass(indexer, NoUsage.class);
        Assert.assertEquals(0, jandexCollector.getUsages().size());
    }
    ...
    public void testClassUsageAndMethodReference() throws Exception {
        JandexCollector jandexCollector = new JandexCollector(runtimeIndex);
        Indexer indexer = new Indexer(jandexCollector);
        scanClass(indexer, ClassUsageAndMethodReference.class);
        Assert.assertEquals(2, jandexCollector.getUsages().size());
        Map<AnnotationUsageType, AnnotationUsage> usages = new HashMap<>();
        for (AnnotationUsage usage : jandexCollector.getUsages()) {
